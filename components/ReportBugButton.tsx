...
export function ReportBugButton({ lang = "english", compact = false, menuItem = false }: { lang?: UiLanguage; compact?: boolean; menuItem?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || tr("Could not send the report."));
      setSent(true);
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : tr("Could not send the report."));
    } finally {
      setBusy(false);
    }
  }
            ...
            <section role="dialog" aria-modal="true" aria-labelledby="bug-report-title" className="issue-desk-enter w-full border border-ink bg-canvas shadow-[12px_12px_0_var(--index-ink)]">
              ...
              {sent ? (
                <div className="result-enter px-5 py-8 sm:px-7">
                  <p className="font-display text-[12px] font-semibold uppercase tracking-[0.08em] text-success"><ShinyText>{tr("Report received")}</ShinyText></p>
                  <p className="mt-3 text-[20px] font-semibold text-ink">{tr("Thank you. It is now on the admin docket.")}</p>
                  <button type="button" onClick={close} className="mt-6 min-h-11 bg-ink px-6 font-display text-[11px] font-semibold uppercase tracking-[0.06em] text-canvas">{tr("Close issue desk")}</button>
                </div>
              ) : (
                <form onSubmit={submit}>
                  <label className="block border-b border-pewter px-5 py-5 text-ash sm:px-7">
                    <span className="micro-label">01 / {tr("Short summary")}</span>
                    <input ref={summaryRef} required maxLength={120} value={summary} onChange={(event) => setSummary(event.target.value)} placeholder={tr("Example: Search button stops responding")} className="mt-3 h-14 w-full border-b-2 border-ink bg-transparent px-0 text-[20px] font-semibold tracking-[-0.02em] text-ink outline-none placeholder:text-stone focus:border-accent" />
                  </label>
                  <label className="block border-b border-pewter px-5 py-5 text-ash sm:px-7">
                    <span className="micro-label">02 / {tr("What happened?")}</span>
                    <textarea required maxLength={1500} rows={5} value={details} onChange={(event) => setDetails(event.target.value)} placeholder={tr("What did you do, what did you expect, and what happened instead?")} className="mt-3 w-full resize-y border border-ink bg-paper px-4 py-3 font-sans text-[14px] font-normal normal-case tracking-normal text-ink outline-none placeholder:text-stone focus:border-accent" />
                  </label>
                  <label className="block px-5 py-5 text-ash sm:px-7">
                    <span className="micro-label">{tr("Reporter name")} <span className="text-stone">/ {tr("optional, private")}</span></span>
                    <input maxLength={80} autoComplete="name" value={reporter} onChange={(event) => setReporter(event.target.value)} placeholder={tr("Helps the admin recognize your report")} className="mt-2 h-11 w-full border border-pewter bg-paper px-3 font-sans text-[13px] font-normal normal-case tracking-normal text-ink outline-none placeholder:text-stone focus:border-accent" />
                  </label>
                  <div className="flex flex-col-reverse gap-2 border-t border-ink px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
                    <p className="text-[10px] uppercase tracking-[0.08em] text-stone">{tr("Page captured · no screenshot or email")}</p>
                    <div className="flex gap-2">
                      <button type="button" onClick={close} className="min-h-11 border border-ink px-5 font-display text-[11px] font-semibold uppercase tracking-[0.06em] text-ink">{tr("Cancel")}</button>
                      <button type="submit" disabled={busy} className="min-h-11 bg-accent px-5 font-display text-[11px] font-semibold uppercase tracking-[0.06em] text-on-accent disabled:opacity-50">{busy ? tr("Sending…") : tr("Send report →")}</button>
                    </div>
                  </div>
