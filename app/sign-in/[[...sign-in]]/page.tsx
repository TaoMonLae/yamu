  ...
  return (
    <main className="grid min-h-screen bg-[#fafaf8] lg:grid-cols-[minmax(0,0.9fr)_minmax(540px,1.1fr)]">
      <section className="relative hidden overflow-hidden bg-[#141415] p-10 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <Link href="/" className="relative flex items-center gap-3 text-white no-underline"><span lang="mnw" className="flex h-10 w-10 items-center justify-center bg-[var(--index-accent)] font-script text-xl font-bold">ယ</span><span><span className="block text-[14px] font-semibold">{branding.siteName}</span><span className="block font-mono text-[9px] uppercase tracking-[0.14em] text-white/45">Catalog operations</span></span></Link>
      ...
      <section className="flex min-h-screen flex-col bg-[#fafaf8] p-5 sm:p-8">
        <div className="flex items-center justify-between"><Link href="/" className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#656661] no-underline">← Public catalog</Link><Link href="/sign-up" className="text-[11px] font-medium text-[#30312f]">Create account</Link></div>
        <div className="flex flex-1 items-center justify-center py-10"><div className="w-full max-w-[430px]"><p className="mb-5 font-mono text-[10px] uppercase tracking-[0.13em] text-[#777873]">Restricted workspace / sign in</p><SignIn routing="path" path="/sign-in" signUpUrl="/sign-up" fallbackRedirectUrl="/admin" /></div></div>
      </section>
    </main>
  );
}
